...
export function brokeredPreviewStorage() {
  ...
  const request = (
    ...
    new Promise((resolve) => {
      ...
      let done = false;
      // The timeout only starts a tick later, but `finish` (defined below) is
      // the only thing that clears it and it can never run before this file's
      // synchronous block completes, so the callback's reference is safe.
