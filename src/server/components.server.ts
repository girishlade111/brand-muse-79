...
export const generateCustomComponentFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<CustomComponentResult> => {
    // 1. Fetch kit tokens, colors, fonts, voice from Drizzle
    const [kitRows, colorRows, fontRows, voiceRows] = await Promise.all([
    ...
    const theme = resolveBrandComponentTheme(
      colorRows,
      fontRows,
      data.themeMode as ComponentThemeMode,
      kit.name,
    );
    ...
    const fallbackTemplate =
      promptLower.includes("checkout") || promptLower.includes("pricing") || promptLower.includes("cart")
        ? DETERMINISTIC_AI_COMPONENTS.checkout
        : DETERMINISTIC_AI_COMPONENTS.testimonial;

    const apiKey = process.env.LOVABLE_API_KEY;
    if (!apiKey) {
      console.warn("[components.server] LOVABLE_API_KEY unconfigured; returning deterministic component.");
      return {
        ok: true,
    ...
    const userPrompt = `Build an accessible, high-contrast UI component for: "${data.prompt}"`;
    ...
    try {
      const result = await callAIStructured<{
        ...
        toolDescription: "Return structured React 19 JSX and HTML snippets styled with brand CSS variables.",
        parameters: {
          type: "object",
          properties: {
      ...
      console.warn("[components.server] AI synthesis failed or rate-limited; engaging fallback:", err?.message);
      return {
        ok: true,
        name: fallbackTemplate.name,
